...
from app.db.crud.order import create_order, get_order_by_id, get_orders_by_user, update_order_status, delete_order
...
@router.put("/{order_id}")
def update_status(order_id: int, status: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    return update_order_status(db, order_id, status)

@router.delete("/{order_id}")
def delete_order_by_id(order_id: int, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
